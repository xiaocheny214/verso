...
def test_register_custom_embedder() -> None:
    def factory(_settings: AppSettings) -> HashingEmbedder:
        return HashingEmbedder(dims=2)

    register_embedder("hashing_test", factory)
    assert get_embedder_factory("hashing_test") is factory
    built = build_embedder(AppSettings(_env_file=None), provider="hashing_test")
    assert isinstance(built, HashingEmbedder)
    assert len(built.embed_query("a")) == 2
